...
function _M.render(state, dispatch)
  ...
  return dom.section({ ["aria-labelledby"] = "users-title" },
    ...
    dom.form({ role = "search", ["aria-label"] = "Kullanıcı filtreleri", class = "flex flex-wrap items-end gap-2 mb-4" },
      ...
      dom.label({ class = "flex flex-col text-xs text-[var(--fg-muted)]" }, "Rol",
        dom.select({ class = select_cls, onchange = function(e) set_filters({ role = e.value or "" }) end },
      ...
      dom.label({ class = "flex flex-col text-xs text-[var(--fg-muted)]" }, "Durum",
        dom.select({ class = select_cls, onchange = function(e) set_filters({ is_active = e.value or "" }) end },
