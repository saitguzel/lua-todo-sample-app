...
function _M.render(state)
  ...
  return dom.section({ ["aria-labelledby"] = "audit-title" },
    ...
    dom.form({ role = "search", ["aria-label"] = "Audit filtreleri", class = "flex flex-wrap items-end gap-2 mb-4" },
      dom.label({ class = "flex flex-col text-xs text-[var(--fg-muted)]" }, "Eylem",
        dom.select({ class = field_cls, onchange = function(e) set_filters({ action = e.value or "" }) end }, action_opts)),
      dom.label({ class = "flex flex-col text-xs text-[var(--fg-muted)]" }, "Durum",
        dom.select({ class = field_cls, onchange = function(e) set_filters({ status = e.value or "" }) end },
      ...
      dom.label({ class = "flex flex-col text-xs text-[var(--fg-muted)]" }, "Varlık türü",
        dom.select({ class = field_cls, onchange = function(e) set_filters({ entity_type = e.value or "" }) end },
          entity_opts)),
